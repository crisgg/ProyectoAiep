...
namespace TicketAtencion.Controladores
{
    public class AGENTESController : Controller
    {
        ...
        {
            List<AGENTES> b = db.AGENTES.ToList();
            var aGENTES = db.AGENTES.Include(a => a.AREA).Include(a => a.COMUNAS).Include(a => a.LOGIN);
            return View(aGENTES.ToList());
        }


        public JsonResult GetAgente() {
            List<AGENTES> b = db.AGENTES.ToList();
            List<AGENTES> c = new List<AGENTES>();

            foreach (var item in b)
            {
                c.Add(new AGENTES { NOMBRE_AGENTE = item.NOMBRE_AGENTE});
            }

            return Json(new { resultado = true, respuesta = c}, JsonRequestBehavior.AllowGet);
